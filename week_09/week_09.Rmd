...
# Notes on Shea (2019)
  ...
  * Some key terms:
    * Dominance: language use—that is, how frequently bilinguals use their languages and how these are divided across domains
    * Proficiency: a person's ability to use language -> And how is it measured? (fluency, lexical retrieval, grammar tests...)
    * Domains of dominance: a way to determine language dominance -> Situations in which the speaker may decide to use one language over the other - if you use the language A for X, you will not use the language B for X
    * Dimensions of dominance: another way to determine language dominance -> Linguistic competence, production, and processing. Thus, fluency of speech, lexical diversity, morphosyntactic knowledge, length of utterances, parsing speed and accuracy, etc.
    * Incomplete acquisition: a way to explain why there are gaps in heritage grammars -> Aspects of the heritage grammar do not have a chance to be acquired due to insufficient L1 input exposure
    * Attrition: another way to explain why there are gaps in heritage grammars -> Aspects of the heritage grammar are acquired but subsequently lost due to interrupted L1 input exposure
    * Limited formal education
    * Divergent attainment: it is essentialy the same as 'incomplete acquisition', but without negative connotations -> The eventual HS adult grammar does not reach native-like (monolingual?) development
    * Structural simplicity: the characteristic of heritage grammars, when some linguistic aspects and contrasts are "lessened" compared to nonheritage native grammars (e.g., Spanish subjunctive)

* For me, what makes this study different is that they analyze different measures within the "main" predictors, to see which of them can better explain variability. For example, for proficiency they don't only use idk a DELE test, but rather 3 different measures. The same is for dominance, they consider, separately, 4 different measures, for example, Spanish use in the home. 
* Also, the statistical analysis. Pilai scores?
    ...
    * Methodology:
        ...
        * Materials:
            ...
            * Fluency tasks (2 monologues)
                * "adult" (current events, university) AND "child" (family, childhood) monologues 
                * They didn't explicit how it was scored
            * Expressive One Word Picture Vocabulary Test (vocabulary knowledge)
            * Picture naming task
